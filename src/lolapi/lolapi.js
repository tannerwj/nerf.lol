      ...
      return new Promise(function (resolve, reject){
        redis.get('summoner:' + name, function (err, reply){
          ...
          Summoner.find({
            ...
            if (!data.length){
              League.Summoner.getByName(name, regionOrFunction).then(function (result){
                //mongo overwrites id, saved custom object instead
                var result = { name: result[name].name, sumId: result[name].id }
                var s = new Summoner(result)
